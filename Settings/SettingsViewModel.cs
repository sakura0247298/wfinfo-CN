    ...
    public class SettingsViewModel : INPC, INotifyDataErrorInfo
    {
        ...
        public int OverlayXOffsetValue
        {
            ...
            set { 
                ...
                {
                    if (_windowInfo.Window.Width == 0 || _windowInfo.Window.Height == 0)
                    {
        ...
        public void Save()
        {
            ...
            try
            {
                ...
                var json = JsonConvert.SerializeObject(ApplicationSettings.GlobalSettings, Formatting.Indented, jsonSettings);
                File.WriteAllText(settingsDirectory, json);
            }
            catch (Exception ex)
            {
                Main.AddLog($"保存设置失败: {ex.Message}");
            }
        }
