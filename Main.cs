...
namespace WFInfo
{
    class Main
    {
        ...
        public Main()
        {
            Main.AddLog("Main 构造函数开始");
            // 在构造函数中初始化所有 WPF 窗口（此时是 STA 线程）
            window = new RewardWindow();
            overlays = new Overlay[4] { new Overlay(), new Overlay(), new Overlay(), new Overlay() };
            equipmentWindow = new EquipmentWindow();
            Main.AddLog($"创建 SettingsWindow 前, Locale: {ApplicationSettings.GlobalSettings.Locale}");
            settingsWindow = new SettingsWindow();
            Main.AddLog($"创建 SettingsWindow 后, Locale: {ApplicationSettings.GlobalSettings.Locale}");
            ...
            AutoUpdater.Start("https://github.com/sakura0247298/wfinfo-CN/releases/latest/download/update.xml");
        ...
        public async void ThreadedDataLoad()
        {
            try
            {
                ...
                OCR.Init(new SoundPlayer(), ApplicationSettings.GlobalReadonlySettings, _windowInfo, _detector, _gdiScreenshot, _windowsScreenshot);
