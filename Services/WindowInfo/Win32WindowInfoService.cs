...
namespace WFInfo.Services.WindowInfo
{
    public class Win32WindowInfoService : IWindowInfoService
    {
        ...
        { 
            get
            {
                // 对于 21:9 屏幕（如 2560x1080），使用高度来计算缩放
                // 因为 Warframe 的 UI 是基于 16:9 设计的，在 21:9 屏幕上会有黑边或居中显示
                double aspectRatio = (double)Window.Width / Window.Height;
                
                if (aspectRatio > 16.0 / 9.0)  // 宽屏（如 21:9）
                {
                    // 对于超宽屏，使用高度计算缩放，因为 UI 是垂直居中的
                    return Window.Height / 1080.0;
                }
                else if (Window.Width * 9 > Window.Height * 16)  // 窄屏（如 4:3）
                {
                    return Window.Height / 1080.0;
                }
                else  // 标准 16:9
        ...
        public double AspectRatio => (double)Window.Width / Window.Height;
        public bool IsUltraWide => AspectRatio > (16.0 / 9.0);
